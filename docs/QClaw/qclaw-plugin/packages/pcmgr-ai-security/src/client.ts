...
import {
  getModerateUrl,
  getModerateSkillUrl,
  getModerateScriptUrl,
  getModerateSkillPackageUrl,
  getPipelineAuditUrl,
} from "./endpoints.js";
import type { PipelineResponse, ToolCallAuditRequest } from "./agentdr/types.js";
...
export class LLMShieldClient {
  ...
  private async postJson(
    ...
    try {
      ...
      // 内网标签：由 Electron 主进程通过 4220 接口探测后注入环境变量，透传给后端
      const innerUser = process.env.QCLAW_INTERNAL_USER === "1" ? "1" : "0";
      headers["Inneruser"] = innerUser;
      fileLog(`[postJson] Inneruser=${innerUser} (QCLAW_INTERNAL_USER=${process.env.QCLAW_INTERNAL_USER ?? "undefined"})`);
  ...
  /**
   * Tool-call Pipeline audit: POST ToolCallAuditRequest to pipelineAudit URL (JPrx 4217 forward).
   */
  async auditPipeline(
    request: ToolCallAuditRequest,
    extraHeaders?: Record<string, string>,
    timeoutOverrideMs?: number
    ...
    return this.postJson(
      ...
      request,
      extraHeaders,
      timeoutOverrideMs ?? 3_000
    ) as Promise<PipelineResponse>;
  }
}

export type { PipelineEnvelope, PipelineResponse } from "./agentdr/types.js";
