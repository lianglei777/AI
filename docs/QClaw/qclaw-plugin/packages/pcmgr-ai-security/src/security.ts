...
import { LLMShieldClient, ContentType, DecisionType, RiskItem } from "./client.js";
import { generateRequestId, recordLogEvent } from "./utils.js";
import { fileLog } from "./logger.js";
import { globalCircuitBreaker, CircuitBreakerConfig } from "./circuit-breaker.js";
...
export async function checkContentSecurity(
  ...
  globalCircuitBreaker.setLogger(logger);
  if (globalCircuitBreaker.isOpen()) {
    const remaining = Math.round(globalCircuitBreaker.remainingCooldownMs() / 1000);
    logger.debug(`[${LOG_TAG}] Circuit-breaker is open, skipping moderation. Remaining cooldown: ${remaining}s`);
