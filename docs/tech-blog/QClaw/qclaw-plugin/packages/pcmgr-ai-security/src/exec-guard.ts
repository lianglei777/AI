...
//
// 注意：不再笼统拦截整个 \appdata\，改为只拦截 appdata 下的高危子目录，
// 避免误拦 Python/Node.js/VS Code 等安装在 AppData\Local\Programs 的合法程序。
// AppData\Local\Microsoft\ 整体不拦截：该目录由操作系统和微软应用管理，
// 包括 WindowsApps（Microsoft Store 安装的 Python、winget 等）、Edge、Teams 等，
// 文件来源可控，不属于用户手动放入的不受信目录。
...
// ---- AppData 下的受信安装子目录段（小写，正斜杠）----
// 这些是 Python、Node.js、VS Code、pnpm 等正规软件的标准安装路径，
// 即使位于 appdata 目录树下也应放行。
// 用于 isInUntrustedDirectory() 中对 appdata 命中后的二次白名单检查。

const TRUSTED_APPDATA_SEGMENTS = [
  "/appdata/local/programs/",       // Python, VS Code, Node.js 等
  "/appdata/roaming/npm/",          // npm 全局安装
  "/appdata/roaming/python/",       // Python 用户安装的 scripts
  "/appdata/local/yarn/",           // Yarn 全局安装
  "/appdata/local/pnpm/",           // pnpm 全局安装
  "/appdata/local/nvs/",            // NVS (Node Version Switcher)
  "/appdata/roaming/nvm/",          // nvm-windows
];
 ...
 *
 * 判断优先级：
 * 1. 受信系统目录前缀（Program Files、Windows 等）→ 放行
 * 2. AppData 受信子目录白名单（Local\Programs 等）→ 放行
 * 3. 不受信目录段匹配（Downloads、Desktop、Temp、AppData 高危子目录等）→ 拦截
 */
function isInUntrustedDirectory(filePath: string): boolean {
  ...
  // Windows 平台：检查 AppData 下的受信安装子目录
  // 如 AppData\Local\Programs\Python 等正规软件安装路径应放行
  if (process.platform === "win32") {
    for (const trustedSeg of TRUSTED_APPDATA_SEGMENTS) {
      if (normalized.includes(trustedSeg)) {
        return false; // AppData 下的受信安装目录，放行
      }
    }
  }
