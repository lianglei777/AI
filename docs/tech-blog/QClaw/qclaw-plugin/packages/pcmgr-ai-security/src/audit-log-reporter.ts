...
import { DEFAULT_AUDIT_REPORT_URL, deriveConfigPullUrl, getSwitches } from "./runtime-config";

/**
 * 原始 fetch 引用（通过 initAuditReporter 注入）。
 *
 * 必须使用原始 fetch（绕过 FetchChain），避免审计上报请求被自身拦截。
 */
let nativeFetch: typeof globalThis.fetch = globalThis.fetch;

/**
 * 初始化审计日志模块，注入原始 fetch 引用。
 * 必须在 FetchChain 安装之前或使用 ctx.getOriginalFetch() 获取的引用调用。
 */
export function initAuditReporter(originalFetch: typeof globalThis.fetch): void {
...
export async function reportAuditLog(entry: AuditLogEntry): Promise<void> {
  ...
  const effectiveUrl = auditReportUrl || DEFAULT_AUDIT_REPORT_URL;
  const reportUrl = deriveConfigPullUrl(effectiveUrl, 'report');
  if (reportUrl) {
