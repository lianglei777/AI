import { SessionType, ResultCode } from "./types.js";
import type { MediaItem, SecurityCheckResult, SecurityConfig, SceneType } from "./types.js";
...
import { getExternalGuid, getExternalUid, getExternalAppVersion, getExternalSourceTerminal, getExternalPromptId, getExternalWechatSessionId, getOpenclawVersion } from "./state.js";
import { QueryScene } from "./utils.js";
...
export const checkContentSecurity = async (
    ...
    const span = tracer.startSpan(
      stageName,
      {
        ...
        attributes: {
          ...
          "openclaw.trace_id": galileoTraceId ?? "",
          "openclaw.query_scene": extra?.query_scene || QueryScene.OTHERS,
          ...extraAttrs,
        },
      },
      resolvedParentCtx,
  ...
  if (isDegraded) {
    ...
    if (now - lastRetryTime > currentRetryIntervalMs && !isProbing) {
      ...
      try {
        ...
        // [diag] 降级恢复
        console.warn(
          `[qclaw-plugin:content-security] [diag] DEGRADED_RECOVERED scene=${scene}` +
          ` source=${source} sessionId=${sessionId} consecutiveFailures=${consecutiveFailures}`,
        );
        ...
        // [diag] 降级探测失败
        console.warn(
          `[qclaw-plugin:content-security] [diag] PROBE_FAILED scene=${scene}` +
          ` source=${source} nextRetryMs=${currentRetryIntervalMs}`,
        );
  ...
  while (attempt < maxAttempts) {
    try {
      // 从 parentCtx 中提取当前 span 的 traceId 和 spanId，注入到审核请求的 traceparent 中
      const parentSpan = resolvedParentCtx !== ROOT_CONTEXT ? trace.getSpan(resolvedParentCtx) : undefined;
      const parentSpanCtx = parentSpan?.spanContext();
      const ctxTraceId = parentSpanCtx?.traceId;
      const ctxSpanId = parentSpanCtx?.spanId;
      const { response, requestId } = await client.createTask(scene, media, sessionId, sessionType, qaid, ctxTraceId, ctxSpanId, galileoTraceId, extra);
      ...
      if (consecutiveFailures >= failureThreshold) {
        ...
        // [diag] 进入降级模式
        console.warn(
          `[qclaw-plugin:content-security] [diag] ENTERED_DEGRADED scene=${scene}` +
          ` source=${source} consecutiveFailures=${consecutiveFailures}` +
          ` threshold=${failureThreshold} error=${error?.message || String(error)}`,
        );
      } else if (isTimeout) {
        // [diag] 审核超时（未达降级阈值）
        console.warn(
          `[qclaw-plugin:content-security] [diag] AUDIT_TIMEOUT scene=${scene}` +
          ` source=${source} attempt=${attempt} consecutiveFailures=${consecutiveFailures}` +
